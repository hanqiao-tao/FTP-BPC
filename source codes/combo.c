...
// modified by Hanqiao Tao, 2025, 12
...
static stype combo(item* f, item* l, stype c, stype lb, stype ub,
    boolean def, boolean relx)
    /* f,l : first, last item                                               */
...
int combo_separation(int n, int* p, int* w, int* x, int c)
{
    ...
    if (_n > 0)
    {
        val = combo(&items[0], &items[_n - 1], c, 0, 5e8, 1, 1);
        ...
        for (i = 0; i < _n; i++)
        {
            ...
            for (j = 0; j < n; j++)
            {
                if (items[i].p == p[j] && items[i].w == w[j] && x[j] == 0)
                {
                    x[j] = 1;
                    break;
                }
